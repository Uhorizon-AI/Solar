...
| Runtime state | Write the execution log under `<runtime root>/task-logs/` and change task status through `solar-state`. | Deleting or rewriting unrelated runtime state. |
