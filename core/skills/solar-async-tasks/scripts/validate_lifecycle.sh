#!/bin/bash

# Validates solar-async-tasks lifecycle: structure, state transitions, and task operations.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=../../solar-client/scripts/resolve_solar_paths.sh
source "$SCRIPT_DIR/../../solar-client/scripts/resolve_solar_paths.sh"
solar_resolve_paths --quiet
cd "$SOLAR_WORKSPACE"
_CORE="$(solar_core_dir)"
_TASKS="$_CORE/skills/solar-async-tasks/scripts"

# Isolate this verification from real runtime data.
TMP_ROOT="$(mktemp -d /tmp/solar-async-tasks-verify.XXXXXX)"
export SOLAR_TASK_ROOT="$TMP_ROOT"
trap 'rm -rf "$TMP_ROOT"' EXIT

# Setup
echo "Running setup in isolated root: $SOLAR_TASK_ROOT"
bash "$_TASKS/setup_async_tasks.sh"

# Create
echo "Creating task..."
OUTPUT=$(bash "$_TASKS/create.sh" "Test Task" "Description")
echo "$OUTPUT"
TASK_ID=$(echo "$OUTPUT" | grep "ID:" | cut -d ' ' -f 2)

if [ -z "$TASK_ID" ]; then
    echo "Failed to create task"
    exit 1
fi

# List drafts
echo "Listing drafts..."
bash "$_TASKS/list.sh" | grep "$TASK_ID"

# Plan
echo "Planning task..."
bash "$_TASKS/plan.sh" "$TASK_ID"

# Approve
echo "Approving task (High Priority)..."
bash "$_TASKS/approve.sh" "$TASK_ID" high

# List queued
echo "Listing queued..."
bash "$_TASKS/list.sh" | grep "$TASK_ID"

# Start
echo "Starting task..."
START_OUT="$(bash "$_TASKS/start_next.sh")"
echo "$START_OUT"
if ! echo "$START_OUT" | grep -q "Started task: \\[$TASK_ID\\]"; then
    echo "Failed: start_next.sh did not activate expected task $TASK_ID"
    exit 1
fi

# List active
echo "Listing active..."
bash "$_TASKS/list.sh" | grep "$TASK_ID"

# Complete
echo "Completing task..."
bash "$_TASKS/complete.sh" "$TASK_ID"

# List completed
echo "Listing completed..."
bash "$_TASKS/list.sh" | grep "$TASK_ID"

echo "Full lifecycle test PASSED."
