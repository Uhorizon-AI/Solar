...
DELIVERY_INSTRUCTION = (
    ...
    "write it the way a colleague who did the work would: plain prose in the "
    "language of the user request, saying what you did and what it means for "
    "them. No labels, no template, no headings. Use bullets when you are "
    "listing things the user has to weigh or decide: buried in a sentence they "
    "get skimmed past. Being clear matters more than being short, so take the "
    "room the work needs and no more.\n"
    "   Name the artifact you acted on, and close with the evidence. When a "
    "link the user can open from this channel exists, give that link: it is "
    "the only evidence they can follow from a phone. When it does not, give a "
    "verifiable reference instead, a relative path or a stable id, and name "
    "the system it lives in, so it can be checked later from a machine. Either "
    "way the evidence is something they can go to; a sentence describing where "
    "the detail lives is not evidence. Do not repeat in the closing line what "
    "you already said in the opening one.\n"
    "   Write about the user's problem, not about your own run. Tooling that "
    "failed on your side belongs in the delivery only when it changed the "
    "answer or left part of the work unverified; say which part, not which "
    "tool.\n"
    "   Raise a decision only when there really is one, and say what you "
    "recommend. Stay under 1200 characters and well under it when the work "
    "allows: what is over that gets cut, and the budget is a ceiling, never a "
    "target. The full account belongs in your reply above the block, not "
    "inside it.\n"
)
...
def _gateway_task_body(
    ...
    return (
        ...
        f"3. Prefer read/analysis and writing declared deliverable paths under the Solar "
        f"workspace. A path that starts with `core/` belongs to the framework, which lives "
        f"at `$SOLAR_ROOT` and is not inside the workspace: resolve it there.\n"
