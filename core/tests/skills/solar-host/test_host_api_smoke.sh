...
APPROVALS="$(curl -sf "http://127.0.0.1:${SOLAR_HOST_PORT}/api/approvals")"
...
REJECT_CODE="$(curl -s -o /tmp/reject_body -w '%{http_code}' -X POST "http://127.0.0.1:${SOLAR_HOST_PORT}/api/approvals/appr_smoke1/reject")"
assert_ok "reject approval" test "$REJECT_CODE" = "200"

REMOVED_CODE="$(curl -s -o /tmp/removed_body -w '%{http_code}' -X POST "http://127.0.0.1:${SOLAR_HOST_PORT}/api/runtime/interface/start")"
