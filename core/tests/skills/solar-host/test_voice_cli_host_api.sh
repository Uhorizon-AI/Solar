...
assert_ok "voice command approve" bash -c "python3 '$SCRIPTS/voice_cli.py' command; curl -sf 'http://127.0.0.1:${SOLAR_HOST_PORT}/api/approvals' | python3 -c \"import json,sys; d=json.load(sys.stdin); a=next(x for x in d['approvals'] if x['approval_id']=='appr_voice1'); assert a['status']!='pending'\""
