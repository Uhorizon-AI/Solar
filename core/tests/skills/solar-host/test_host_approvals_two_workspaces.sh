...
curl -sf "http://127.0.0.1:${SOLAR_HOST_PORT}/api/approvals" >/tmp/appr_a.json
...
SW_CODE="$(curl -s -o /tmp/sw_body -w '%{http_code}' -X POST "http://127.0.0.1:${SOLAR_HOST_PORT}/api/workspaces/active" \
  -H "Content-Type: application/json" -d "{\"path\":\"$WS_B\"}")"
assert_ok "switch to B" test "$SW_CODE" = "200"

curl -sf "http://127.0.0.1:${SOLAR_HOST_PORT}/api/approvals" >/tmp/appr_b.json
...
REJECT_CODE="$(curl -s -o /tmp/rej_b -w '%{http_code}' -X POST \
  "http://127.0.0.1:${SOLAR_HOST_PORT}/api/approvals/appr_ws_b/reject")"
assert_ok "reject on B" test "$REJECT_CODE" = "200"

SW_A="$(curl -s -o /tmp/sw_a -w '%{http_code}' -X POST "http://127.0.0.1:${SOLAR_HOST_PORT}/api/workspaces/active" \
  -H "Content-Type: application/json" -d "{\"path\":\"$WS_A\"}")"
assert_ok "switch back to A" test "$SW_A" = "200"

curl -sf "http://127.0.0.1:${SOLAR_HOST_PORT}/api/approvals" >/tmp/appr_a2.json
