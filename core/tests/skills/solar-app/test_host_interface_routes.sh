...
READY="$(curl -sf "http://127.0.0.1:${SOLAR_APP_PORT}/ready")"
assert_ok "GET /ready" bash -c "echo '$READY' | python3 -c \"import json,sys; d=json.load(sys.stdin); assert 'checks' in d\""

THREAD_RESP="$(curl -sf -X POST "http://127.0.0.1:${SOLAR_APP_PORT}/threads" \
...
GET_THREAD="$(curl -sf "http://127.0.0.1:${SOLAR_APP_PORT}/threads/${THREAD_ID}")"
...
RUN_CODE="$(curl -s -o /tmp/run_body -w '%{http_code}' -X POST \
  "http://127.0.0.1:${SOLAR_APP_PORT}/threads/${THREAD_ID}/runs" \
  -H 'Content-Type: application/json' \
...
if [[ -n "$RUN_ID" ]]; then
  GET_RUN="$(curl -sf "http://127.0.0.1:${SOLAR_APP_PORT}/runs/${RUN_ID}")"
...
DEL_CODE="$(curl -s -o /tmp/del_body -w '%{http_code}' -X DELETE \
  "http://127.0.0.1:${SOLAR_APP_PORT}/threads/${THREAD_ID}")"
