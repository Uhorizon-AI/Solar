...
WS="$TMP/ws"
mkdir -p "$WS/sun" "$WS/core/skills/solar-client/scripts"
WS="$(cd "$WS" && pwd -P)"

cat >"$WS/core/skills/solar-client/scripts/solar" <<'MOCK'
#!/usr/bin/env bash
set -euo pipefail
case "${1:-} ${2:-}" in
...
MOCK
chmod +x "$WS/core/skills/solar-client/scripts/solar"
...
HOST_CODE="$(curl -s -o /tmp/ca_host.json -w '%{http_code}' -X POST "${BASE}/api/actions/client" \
  -H "Content-Type: application/json" \
  -H "Host: evil.example:${SOLAR_APP_PORT}" \
  -d '{"action":"sync"}')"
